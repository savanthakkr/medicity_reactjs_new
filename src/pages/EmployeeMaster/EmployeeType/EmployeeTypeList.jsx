...
const EmployeeTypeList = () => {
	...
	const canView = can(PERM.EMPLOYEE_TYPE.LIST);
	const canAdd = canAll(PERM.EMPLOYEE_TYPE.LIST, PERM.EMPLOYEE_TYPE.ADD);
	const canEdit = canAll(PERM.EMPLOYEE_TYPE.LIST, PERM.EMPLOYEE_TYPE.EDIT);
	const canDelete = canAll(PERM.EMPLOYEE_TYPE.LIST, PERM.EMPLOYEE_TYPE.DELETE);

	const { data, loading, mutate } = useAutoRevalidate(API.EMPLOYEE_TYPES.LIST, {
		page,
		limit: pageSize,
		search,
	...
	const handleDelete = async id => {
		confirm({
			...
			onConfirm: async () => {
				try {
					await http.post(API.EMPLOYEE_TYPES.DELETE(id));
					mutate();
				} catch (error) {
	...
	const handleStatusToggle = async item => {
		...
		setTogglingId(item.employee_type_Id);
		try {
			const res = await http.put(API.EMPLOYEE_TYPES.STATUS(item.employee_type_Id));
			const updatedList = data?.list?.map(d =>
				d.employee_type_Id === item.employee_type_Id ? { ...d, is_active: newStatus } : d
			);
			if (updatedList) {
	...
	const handleDownloadExcel = async () => {
		await downloadExcel({
			url: API.EMPLOYEE_TYPES.DOWNLOAD_EXCEL,
			fileName: 'employee-types.xlsx',
	...
	const columns = useMemo(
		() => [
			...
			},
			{
				...
				render: item => <span className="font-bold text-text-1">{item.employee_type_Name}</span>
			},
			{
				key: 'employee_type_Description',
				label: 'Description',
				widthClassName: 'min-w-[300px]',
				render: item => (
					<span className="text-text-2">
						{item.employee_type_Description || <span className="italic text-text-3">—</span>}
					</span>
				)
			},
			{
				...
				render: item =>
					canAll(PERM.EMPLOYEE_TYPE.LIST, PERM.EMPLOYEE_TYPE.STATUS) ? (
						<Switch
							checked={item.is_active === 1}
							disabled={togglingId === item.employee_type_Id}
			...
			},
			{
				...
				render: item => (
					<TableActions
						onEdit={
							canEdit ? () => navigate(ROUTES.EDIT_EMPLOYEE_TYPE.replace(':id', item.employee_type_Id)) : undefined
						}
						onDelete={canDelete ? () => handleDelete(item.employee_type_Id) : undefined}
					/>
				)
			}
		],
	...
	return (
		<TableLayout
			...
			addLabel={canAdd ? 'Add Employee Type' : undefined}
			addAction={canAdd ? ROUTES.ADD_EMPLOYEE_TYPE : undefined}
			extraAction={
				canAll(PERM.EMPLOYEE_TYPE.LIST, PERM.EMPLOYEE_TYPE.EXCEL) ? (
					<Button
						variant="unstyled"
			...
			filterContent={
				<div className="flex items-center gap-2">
					<TableSearch onSearch={handleSearch} placeholder="Search by Employee Type" />
