...
const DepartmentList = () => {
	...
	const canView = can(PERM.DEPARTMENT.LIST);
	...
	const canDelete = canAll(PERM.DEPARTMENT.LIST, PERM.DEPARTMENT.DELETE);

	const { data, loading, mutate } = useAutoRevalidate(API.DEPARTMENTS.LIST, {
		page,
		limit: pageSize,
		search,
	...
	const handleDelete = async id => {
		confirm({
			...
			onConfirm: async () => {
				try {
					await http.delete(API.DEPARTMENTS.DELETE(id));
	...
	const handleStatusToggle = async item => {
		...
		setTogglingId(item.department_Id);
		try {
			const res = await http.put(API.DEPARTMENTS.STATUS(item.department_Id));
			const updatedList = data?.list?.map(d =>
				d.department_Id === item.department_Id ? { ...d, is_active: newStatus } : d
			);
			if (updatedList) {
	...
	const handleDownloadExcel = async () => {
		await downloadExcel({
			url: API.DEPARTMENTS.DOWNLOAD_EXCEL,
	...
	const columns = useMemo(
		() => [
			...
			},
			{
				...
				render: item => <span className="font-bold text-text-1">{item.department_Name}</span>
			},
			{
				...
				render: item => (
					<span className="text-text-2">
						{item.department_Description || <span className="italic text-text-3">—</span>}
					</span>
				)
			},
			{
				...
				render: item =>
					canAll(PERM.DEPARTMENT.LIST, PERM.DEPARTMENT.STATUS) ? (
						<Switch
							checked={item.is_active === 1}
							disabled={togglingId === item.department_Id}
			...
			},
			{
				...
				render: item => (
					<TableActions
						onEdit={canEdit ? () => navigate(ROUTES.EDIT_DEPARTMENT.replace(':id', item.department_Id)) : undefined}
						onDelete={canDelete ? () => handleDelete(item.department_Id) : undefined}
					/>
				)
			}
		],
	...
	return (
		<TableLayout
			title="Departments"
			addLabel={canAdd ? 'Add Department' : undefined}
			addAction={canAdd ? ROUTES.ADD_DEPARTMENT : undefined}
			extraAction={
				canAll(PERM.DEPARTMENT.LIST, PERM.DEPARTMENT.EXCEL) ? (
					<Button
						variant="unstyled"
			...
			filterContent={
				<div className="flex items-center gap-2">
					<TableSearch onSearch={handleSearch} placeholder="Search by Department Name" />
