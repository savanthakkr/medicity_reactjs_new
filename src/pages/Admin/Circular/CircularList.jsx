...
const CircularList = () => {
	...
	const [sortConfig, setSortConfig] = useState({ key: 'circular_Id', direction: 'desc' });
	...
	const { data, loading, mutate } = useAutoRevalidate(API.CIRCULARS.LIST, {
		page,
		limit: pageSize,
	...
	const handleDelete = async id => {
		confirm({
			...
			onConfirm: async () => {
				try {
					const res = await http.delete(API.CIRCULARS.DELETE(id));
					addToast({ type: 'success', message: res?.data?.msg || 'Circular deleted successfully' });
					...
					addToast({ type: 'error', message: error?.response?.data?.message || 'Failed to delete circular' });
				}
			}
		});
	};
	...
	const handleStatusToggle = async (id, currentStatus) => {
		...
		setTogglingId(id);
		try {
			await http.put(API.CIRCULARS.STATUS(id), { inputData: { is_active: newStatus } });
			const updatedList = data?.list?.map(item => (item.circular_Id === id ? { ...item, is_active: newStatus } : item));
			if (updatedList) {
			...
			},
			{
				...
				render: item => (
					<Switch
						...
						disabled={togglingId === item.circular_Id}
						onChange={() => handleStatusToggle(item.circular_Id, item.is_active)}
					/>
				)
			},
			{
				...
				render: item => (
					<TableActions
						onEdit={() => navigate(ROUTES.EDIT_CIRCULAR.replace(':id', item.circular_Id))}
						onDelete={() => handleDelete(item.circular_Id)}
					/>
				)
			}
		],
	...
	return (
		<TableLayout
			...
			addAction={ROUTES.ADD_CIRCULAR}
			filterContent={
				<div className="flex items-center gap-2">
					<TableSearch onSearch={handleSearch} placeholder="Search Circulars" />
		...
		>
			<CommonTable
				columns={columns}
				sortableColumns={['circular_Name', 'subject', 'document_No']}
