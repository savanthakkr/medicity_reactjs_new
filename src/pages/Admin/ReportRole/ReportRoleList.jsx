...
const ReportRoleList = () => {
	...
	const { data, loading, mutate } = useAutoRevalidate(API.REPORT_ROLES.LIST, {
		page,
		limit: pageSize,
	...
	const handleDelete = async id => {
		confirm({
			title: 'Delete Report Role',
			message: 'Are you sure you want to delete this report role?',
			confirmLabel: 'Delete',
			onConfirm: async () => {
				try {
					const res = await http.delete(API.REPORT_ROLES.DELETE(id));
					addToast({ type: 'success', message: res?.data?.msg || 'Report role deleted successfully' });
					...
					addToast({ type: 'error', message: error?.response?.data?.message || 'Failed to delete report role' });
				}
			}
		});
	};
	...
	const handleStatusToggle = async (id, currentStatus) => {
		...
		setTogglingId(id);
		try {
			await http.put(API.REPORT_ROLES.STATUS(id), { inputData: { is_active: newStatus } });
			const updatedList = data?.list?.map(item =>
				item.report_role_Id === id ? { ...item, is_active: newStatus } : item
			);
			if (updatedList) {
	...
	const columns = useMemo(
		() => [
			...
			{
				key: 'report_role_Description',
				label: 'Description',
				widthClassName: 'min-w-[300px]',
				render: item => <span>{item.report_role_Description || '—'}</span>
			},
			{
				key: 'status',
				label: 'Status',
				...
				render: item => (
					<Switch
						...
						disabled={togglingId === item.report_role_Id}
						onChange={() => handleStatusToggle(item.report_role_Id, item.is_active)}
					/>
				)
			},
			{
				...
				render: item => (
					<TableActions
						onEdit={() => navigate(ROUTES.EDIT_REPORT_ROLE.replace(':id', item.report_role_Id))}
						onDelete={() => handleDelete(item.report_role_Id)}
					/>
				)
			}
		],
	...
	return (
		<TableLayout
			title="Report Roles"
			addLabel="Add Report Role"
			addAction={ROUTES.ADD_REPORT_ROLE}
			filterContent={
				<div className="flex items-center gap-2">
					<TableSearch onSearch={handleSearch} placeholder="Search Report Roles" />
		...
		>
			<CommonTable
				columns={columns}
				sortableColumns={['report_role_Name', 'report_role_Description']}
