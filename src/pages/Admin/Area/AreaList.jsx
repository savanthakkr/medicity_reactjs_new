...
import DateTimeCell from '../../../components/common/DateTimeCell.jsx';
...
const AreaList = () => {
	...
	const [sortConfig, setSortConfig] = useState({ key: 'area_Id', direction: 'desc' });
	const [togglingId, setTogglingId] = useState(null);
	const [search, setSearch] = useState('');

	const canView = can(PERM.AREA.LIST);
	const canAdd = canAll(PERM.AREA.LIST, PERM.AREA.ADD);
	const canEdit = canAll(PERM.AREA.LIST, PERM.AREA.EDIT);
	const canDelete = canAll(PERM.AREA.LIST, PERM.AREA.DELETE);
	const canStatus = canAll(PERM.AREA.LIST, PERM.AREA.STATUS);
	...
	const { data, loading, mutate } = useAutoRevalidate(API.AREAS.LIST, {
		page,
		limit: pageSize,
	...
	const handleDelete = async id => {
		confirm({
			...
			onConfirm: async () => {
				try {
					const res = await http.delete(API.AREAS.DELETE(id));
					addToast({ type: 'success', message: res?.msg || 'Area deleted successfully' });
					...
					addToast({ type: 'error', message: error?.response?.data?.message || 'Failed to delete area' });
				}
			}
		});
	};
	...
	const handleStatusToggle = async (id, currentStatus) => {
		...
		setTogglingId(id);
		try {
			const res = await http.put(API.AREAS.STATUS(id), { inputData: { is_active: newStatus } });
			// Optimistic update
			const updatedList = data?.list?.map(item => (item.area_Id === id ? { ...item, is_active: newStatus } : item));
			if (updatedList) {
	...
	const columns = useMemo(
		() => [
			...
			{
				key: 'collection_Charge',
				label: 'Collection Charge',
				widthClassName: 'min-w-[180px]',
				render: item => <span>{item.collection_Charge}</span>
			},
			{
				key: 'delivery_Charge',
				label: 'Delivery Charge',
				widthClassName: 'min-w-[180px]',
				render: item => <span>{item.delivery_Charge}</span>
			},
			{
				key: 'state_Name',
				label: 'State',
			...
			},
			{
				...
				label: 'Zone',
				widthClassName: 'min-w-[150px]',
				render: item => <span>{item.zone_Name || '—'}</span>
			},

			{
				...
				render: item => (
					<Switch
						...
						disabled={!canStatus || togglingId === item.area_Id}
						onChange={() => canStatus && handleStatusToggle(item.area_Id, item.is_active)}
					/>
				)
			},
			{
				...
				render: item => (
					<TableActions
						onEdit={canEdit ? () => navigate(ROUTES.EDIT_AREA.replace(':id', item.area_Id)) : undefined}
						onDelete={canDelete ? () => handleDelete(item.area_Id) : undefined}
					/>
				)
			}
		],
	...
	return (
		<TableLayout
			title="Areas"
			addLabel={canAdd ? 'Add Area' : undefined}
			addAction={canAdd ? ROUTES.ADD_AREA : undefined}
			extraAction={
				<Button
					variant="unstyled"
			...
			filterContent={
				<div className="flex items-center gap-2">
					<TableSearch onSearch={handleSearch} placeholder="Search Areas" />
		...
		>
			<CommonTable
				columns={columns}
				sortableColumns={['area_Name', 'collection_Charge', 'delivery_Charge', 'state_Name', 'zone_Name']}
