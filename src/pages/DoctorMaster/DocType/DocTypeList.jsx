...
const DocTypeList = () => {
	...
	const [sortConfig, setSortConfig] = useState({ key: 'doc_type_Id', direction: 'desc' });
	...
	const canView = can(PERM.DOCTOR_TYPE.LIST);
	...
	const canDelete = canAll(PERM.DOCTOR_TYPE.LIST, PERM.DOCTOR_TYPE.DELETE);
	const canStatus = canAll(PERM.DOCTOR_TYPE.LIST, PERM.DOCTOR_TYPE.STATUS);
	const canExcel = canAll(PERM.DOCTOR_TYPE.LIST, PERM.DOCTOR_TYPE.EXCEL);
	...
	const { data, loading, mutate } = useAutoRevalidate(API.DOC_TYPE.LIST, {
		page,
		limit: pageSize,
	...
	const handleDelete = async id => {
		confirm({
			title: 'Delete Doctor Type',
			message: 'Are you sure you want to delete this doctor type?',
			confirmLabel: 'Delete',
			onConfirm: async () => {
				try {
					const res = await http.delete(API.DOC_TYPE.DELETE(id));
					addToast({ type: 'success', message: res?.data?.msg || 'Doctor type deleted successfully' });
					...
					addToast({
						type: 'error',
						message: error?.response?.data?.msg || error?.response?.data?.message || 'Failed to delete doctor type'
					});
				}
			}
		});
	...
	const handleStatusToggle = async (id, currentStatus) => {
		...
		setTogglingId(id);
		try {
			await http.put(API.DOC_TYPE.STATUS(id), { inputData: { is_active: newStatus } });
			const updatedList = data?.list?.map(item => (item.doc_type_Id === id ? { ...item, is_active: newStatus } : item));
			if (updatedList) {
	...
	const handleDownloadExcel = async () => {
		await downloadExcel({
			url: API.DOC_TYPE.DOWNLOAD_EXCEL,
			fileName: 'doc_types.xlsx'
		});
	};
		...
		() => {
			const cols = [
				...
				},
				{
					...
					render: item => <span className="font-semibold text-text-1">{item.doc_type_Name}</span>
				},
				{
					key: 'doc_type_Description',
					label: 'Description',
					widthClassName: 'min-w-[250px]',
					render: item => (
						<span className="text-text-2">
							{item.doc_type_Description || <span className="italic text-text-3">—</span>}
						</span>
					)
				},
				{
					...
					render: item => (
						<Switch
							...
							disabled={!canStatus || togglingId === item.doc_type_Id}
							onChange={() => {
								if (!canStatus) return;
								handleStatusToggle(item.doc_type_Id, item.is_active);
							}}
						/>
					)
				}
			...
			if (canEdit || canDelete) {
				cols.push({
					...
					render: item => (
						<TableActions
							onEdit={canEdit ? () => navigate(ROUTES.EDIT_DOCTOR_MASTER_DOC_TYPE.replace(':id', item.doc_type_Id)) : undefined}
							onDelete={canDelete ? () => handleDelete(item.doc_type_Id) : undefined}
						/>
					)
				});
			}
	...
	return (
		<TableLayout
			title="Doctor Types"
			addLabel={canAdd ? "Add Doctor Type" : undefined}
			addAction={canAdd ? ROUTES.ADD_DOCTOR_MASTER_DOC_TYPE : undefined}
			extraAction={
				canExcel ? (
					<Button
						variant="unstyled"
		...
		>
			<CommonTable
				columns={columns}
				sortableColumns={['doc_type_Name', 'doc_type_Description']}
