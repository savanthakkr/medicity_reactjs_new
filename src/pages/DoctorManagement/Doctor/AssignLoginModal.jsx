...
const AssignLoginModal = ({ open, onClose, doctor, onSuccess }) => {
	...
	const isEdit = Boolean(doctor?.user_Id);
	...
	useEffect(() => {
		...
		// Auto-generate username: "{doc_Code}_{doc_Name}" e.g. "doc_2026_00001_savan_john_doe"
		const autoUsername = (() => {
			const code = (doctor?.doc_Code || '')
				.trim()
				.toLowerCase()
				.replace(/[\/\s]+/g, '_');
			const name = (doctor?.doc_Name || '')
				.trim()
				.toLowerCase()
				.replace(/\s+/g, '_')
				.replace(/[^a-z0-9._-]/g, '');
		...
		const load = async () => {
			...
			try {
				...
				if (doctor?.user_Id) {
					const res = await http.post(API.DOCTORS.LOGIN_DETAILS(doctor.doc_Id));
		...
		load();
	}, [open, doctor?.doc_Id, doctor?.user_Id, clientId]);
	...
	const handleSubmit = async e => {
		...
		setLoading(true);
		try {
			await http.post(API.DOCTORS.ASSIGN_LOGIN(doctor.doc_Id), {
			...
			console.error(err);
			const msg = err?.response?.data?.msg || 'Operation failed. Please try again.';
	...
	return (
		<Modal
			open={open}
			onClose={onClose}
			title={isEdit ? `Edit Login — ${doctor?.doc_Name}` : `Assign Login — ${doctor?.doc_Name}`}
					...
					<div className="flex flex-col gap-4">
						{isEdit && (
							<div className="rounded-[6px] bg-blue-50 px-3 py-2 text-[12px] text-blue-700">
								This doctor already has login access. Leave <strong>password</strong> blank to keep the current one.
							</div>
						)}
						...
						<div>
							...
							{!errors.user_Username && (
								<p className="mt-1 text-[11px] text-text-3">
									Auto-generated from doctor code + name — you can edit it.
								</p>
							)}
						</div>
